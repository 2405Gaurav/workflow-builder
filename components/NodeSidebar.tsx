...
export function NodeSidebar() {
  ...
  return (
    <motion.div 
      ...
      className="h-full flex flex-col bg-[#050505] border-r border-white/5 overflow-hidden select-none relative z-30 shadow-2xl"
    >
      {/* 1. TOP HEADER & COLLAPSE TOGGLE */}
      <div className={`flex items-center justify-between px-3 pt-3 pb-1 ${isCollapsed ? 'flex-col gap-4' : ''}`}>
      ...
      {/* 3. SEARCH BAR (Only visible when expanded) */}
      {!isCollapsed && <NodeSearch query={searchQuery} setQuery={setSearchQuery} />}

      {/* 4. NODE LIBRARY */}
      <div className={`flex-1 overflow-y-auto px-4 custom-scrollbar ${isCollapsed ? 'flex flex-col items-center' : ''}`}>
        <AnimatePresence>
          {!isCollapsed && (
            <motion.h3 
              initial={{ opacity: 0 }} 
        ...
        <div className="flex flex-col gap-1 w-full">
          {filteredNodes.map((config, idx) => (
            <motion.button
              ...
              className={`
                group flex items-center transition-all cursor-grab active:cursor-grabbing rounded-xl
                ${isCollapsed ? 'justify-center p-2' : 'gap-3 p-2 w-full text-left'}
              `}
            >
      ...
      {/* 5. USER FOOTER */}
      <div className={`p-4 bg-[#080808] border-t border-white/5 ${isCollapsed ? 'flex justify-center' : ''}`}>
        <div className={`flex items-center p-2 rounded-2xl bg-white/[0.02] border border-white/5 ${isCollapsed ? 'w-fit' : 'gap-3'}`}>
    ...
    <Link href={href}>
      ...
      >
        <div className={`
          ...
          ${isActive ? 'bg-white text-black shadow-2xl shadow-white/20' : 'bg-[#111] text-white/30 group-hover:text-white/60'}
        `}>
